...
const workerUrl = (CONFIG.booking.workerUrl || "").replace(/\/$/, "");
if (!workerUrl) {
...
$("#fSubmit").onclick = async () => {
  ...
  try {
    ...
    const res = await fetch(workerUrl + "/book", {
      ...
      body: JSON.stringify({ ...b, docNo, filename, pdfBase64: b64(pdfBytes) }),
    });
    ...
    const a = $("#donePdf"); a.href = url; a.download = filename; a.textContent = "▼ " + t("pdf");
  } catch (e) {
    $("#bkProg").style.display = "none";
