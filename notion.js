...
function effWorkerUrl(s) {
  return (s.relayCustom || CONFIG.workerUrl || "").replace(/\/$/, "");
}
function effRelay(s) {
  // Prefer the worker whenever it can actually work: explicit choice, or
  // no local token (public relays can't inject one — the worker can).
  const w = effWorkerUrl(s);
  if (s.relay === "worker" && w) return "worker";
  if (!s.token && w) return "worker";
  return s.relay || "corsproxy";
}
export function isConnected() {
  const s = loadSettings();
  if (s.demo) return false;
  if (s.token) return true;
  return effRelay(s) === "worker" && !!effWorkerUrl(s); // worker holds the token
}
...
function relayUrl(url) {
  ...
  return r.build(url, effWorkerUrl(s));
}

async function api(path, { method = "GET", body, isForm = false } = {}) {
  const s = loadSettings();
  const url = "https://api.notion.com" + path;
  const headers = { "Notion-Version": CONFIG.notionVersion };
  if (s.token) headers.Authorization = "Bearer " + s.token;
  else if (s.staffKey) headers["X-Staff-Key"] = s.staffKey;
  // no local token + worker relay → the Worker injects its stored secret
