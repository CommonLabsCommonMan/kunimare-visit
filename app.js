...
import { generateReportPdf, makeDocNo } from "./pdfgen.js";
...
function renderShell() {
  ...
  <div class="shell">
    <aside class="side">
      ...
      <div class="foot">© Kunimare Brewery<br>来訪管理 v1.0</div>
    </aside>
...
async function viewReserve(id) {
  ...
  main.innerHTML = pageHead(v ? "resEditTitle" : "resTitle", "Visit reservation") + banners() + `
  ...
  const auto = () => {
    ...
    if (c || n) ti.value = `${c}${c && n ? " " : ""}${n}${n ? "" : ""} 来社`.trim();
  };
  ...
  <div class="cbody">
    <div class="fgrid">
      ...
      <div class="field"><label>${t("setRelay")}</label><select id="sRelay">
        ${CONFIG.relays.map((r) => `<option value="${r.id}" ${(S.relay || "corsproxy") === r.id ? "selected" : ""}>${lang === "ja" ? r.labelJa : r.labelEn}</option>`).join("")}
      </select></div>
      <div class="field"><label>${t("setRelayUrl")}</label>
        <input type="text" id="sRelayUrl" value="${esc(S.relayCustom || "")}" placeholder="${esc(t("setRelayUrl_ph"))}"></div>
